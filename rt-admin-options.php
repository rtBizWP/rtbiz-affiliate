            ...
            if ($_POST['project_status'] == 'completed') {
                foreach ($_POST['record'] as $id) {
                    ...
                    if ($row->id == NULL) {
                        ...
                        $amt = 0;
                        if ($row_contact->service_b2w_migration == 'yes')
                            $amt += RT_AFFILIATE_COMMISSION_B2W;
                        if ($row_contact->service_wp_theme == 'yes')
                            $amt += RT_AFFILIATE_COMMISSION_THEME;
        ...
        if ($_POST) {
            ...
            if ($_POST['project_status'] == 'completed') {
                ...
                if ($row->id == NULL) {
                    ...
                    if ($row_contact->service_b2w_migration == 'yes')
                        $amt += RT_AFFILIATE_COMMISSION_B2W;
                    if ($row_contact->service_wp_theme != 'no')
                        $amt += RT_AFFILIATE_COMMISSION_THEME;
...
function rt_affiliate_options_manage_payment() {
    ?>
        ...
        if (isset($_GET['action']) && ( $_GET['action'] == 'add' || $_GET['action'] == 'edit'))
            rt_affiliate_options_manage_payment_edit();
        else
            rt_affiliate_options_manage_payment_list();
        ?></div><?php
}
