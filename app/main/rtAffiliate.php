            ...
            if ( isset ( $_GET[ 'ref' ] ) ) {
                ...
                if ( $row ) {
                    ...
                    if(isset($_SERVER[ 'HTTP_REFERER' ])){
                        ...
                        if( !$d_row ){
                            $wpdb->insert($wpdb->prefix . "rt_aff_users_domain", array( "user_id" => $row->ID , "domain_name" => $domain_name , "status" => 'y' ),
                                    array('%d','%s','%s'));
                        } else {
                            if( isset( $d_row->status ) && ( $d_row->status == "n" )){
                                $set_cookies_flag = false;
                            }else{
                                $wpdb->update($wpdb->prefix . "rt_aff_users_domain", array('count' => $d_row->count + 1 ), array( "user_id" => $row->ID , "domain_name" => $domain_name ),
                                        array('%d'),array('%d','%s'));
                            }
                        }
