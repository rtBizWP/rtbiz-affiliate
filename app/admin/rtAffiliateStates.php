...
class rtAffiliateStates extends WP_List_Table {
	public function __construct() {
		...
		parent::__construct( array(
			'ajax' => true, 'plural' => 'stateshistory', 'singular' => 'statesthistory',
		) );
	}
	...
	function get_columns() {
		return array(
			'id' => __( '#' ), 'date' => __( 'Date & Time' ), 'referred_from' => __( 'Referred From' ), 'ip_address' => __( 'Location' ), 'landing_page' => __( 'Landing Page' ),
		);
	}

	function get_sortable_columns() {
		$sortable_columns = array(
			'id' => array( 'id', false ), 'date' => array( 'payment_method', true ),
		);
	...
	function column_referred_from( $row ) {
		if ( $row->referred_from != '' ) {
			echo '<a target="_blank" href="' . $row->referred_from . '">' . $row->referred_from . '</a>';
		} else {
			echo 'No Link';
		}
	}
	...
	function column_landing_page( $row ) {
		if ( $row->landing_page != '' ) {
			echo '<a target="_blank" href="' . $row->landing_page . '">' . $row->landing_page . '</a>';
		} else {
			echo 'No Link';
		}
	}
