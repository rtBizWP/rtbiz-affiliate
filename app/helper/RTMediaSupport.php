...
if ( ! class_exists( 'RTMediaSupport' ) ) {
	...
	class RTMediaSupport {
		...
		public function get_support_content() {
			...
			$tabs[ ] = array(
				'title' => 'Premium Support', 'name' => __( 'Premium Support', 'rtmedia' ), 'href' => '#support', 'callback' => array( $this, 'call_get_form' )
			);
			$tabs[ ] = array(
				'title' => 'Debug Info', 'name' => __( 'Debug Info', 'rtmedia' ), 'href' => '#debug', 'callback' => array( $this, 'debug_info_html' )
			);
			$tabs[ ] = array(
				'title' => 'Migration', 'name' => __( 'Migration', 'rtmedia' ), 'href' => '#migration', 'callback' => array( $this, 'migration_html' )
			);
			?>
			<div id="rtm-support">
				<ul>
					...
					foreach ( $tabs as $tab ) {
						?>
						<li><a id="tab-<?php echo substr( $tab[ 'href' ], 1 ); ?>" title="<?php echo $tab[ 'title' ] ?>"
							   href="<?php echo $tab[ 'href' ]; ?>"
							   class="rtmedia-tab-title"><?php echo $tab[ 'name' ]; ?></a></li>
					<?php
					}
					?>
				</ul>
